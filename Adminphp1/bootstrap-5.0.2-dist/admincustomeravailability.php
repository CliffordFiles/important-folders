...
if(!empty($_POST["newmobilenumber"])) {
  $query = "SELECT * FROM registration WHERE mobilenumber = :mobilenumber";
  $stmt = $conn->prepare($query);
  $stmt->bindParam(':mobilenumber', $_POST["newmobilenumber"], PDO::PARAM_STR);
  $stmt->execute();
  $count = $stmt->rowCount();
  if($count > 0) {
...
if(!empty($_GET["mobilenumber"])) {
  $query = "SELECT * FROM registration WHERE mobilenumber = :mobilenumber";
  $stmt = $conn->prepare($query);
  $stmt->bindParam(':mobilenumber', $_GET["mobilenumber"], PDO::PARAM_STR);
  $stmt->execute();
  $count = $stmt->rowCount();
  if($count > 0) {
    echo "already_used";
